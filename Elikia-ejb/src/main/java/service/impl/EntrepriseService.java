...
public class EntrepriseService implements EntrepriseServiceLocal,EntrepriseServiceRemote {
	...
	public void removePublication(int idcand) {
		em.find(Publication.class, idcand).getEntreprisepub().getPublications().remove(em.find(Publication.class, idcand));
		em.remove(em.find(Publication.class, idcand)); 
	}
	...
	public void affecterPublicationAEntreprise(int entId,Publication pub) {
		Entreprise entreprise=em.find(Entreprise.class, entId);
		entreprise.addPub(pub);
	}
	public List<Publication> findAllPub(int id) {
		List<Publication> users = em.createQuery("select e from Publication e where e.entreprisepub.id=:id and e.importance < 4 order by e.datepub DESC", Publication.class).setParameter("id", id).getResultList(); 
		return users;
	}
	...
	public List<Publication> findAllPubMost(int id) {
		List<Publication> users = em.createQuery("select e from Publication e where e.entreprisepub.id=:id and e.importance = 5 order by e.datepub DESC", Publication.class).setParameter("id", id).getResultList(); 
		return users;
	}
	...
	public void deletePublicationById(int PubId) {
		em.remove(em.find(Publication.class,PubId)); 
	}
	public void updatePublication(int id ,String contenu,int importance , String titre){
		Publication user = em.find(Publication.class, id);
